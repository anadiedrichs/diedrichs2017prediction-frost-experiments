...
#' 
#' Para entrenar se utilizaron sólo los meses de agosto, setiembre, octubre, noviembre
#' y se desfasaron los datos a dos días, es decir, contamos sólo con variables de dos días previos para predecir
#' la temperatura mínima / helada del día siguiente.
#
...
#' datos de los meses agosto, setiembre, octubre y noviembre
sensores <- dacc.spring()
...
#' Obtengo dataset con variables desfasadas a un dia
#' 
aux <- desfasar.dataset.t2(sensores, pred_sensores)
...
#' 
save(res, file = paste("hc-dacc-spring-t2-spring-",Sys.time(),".RData",sep=""))
...
#' guardar modelo 
save(res, file= paste("hc-dacc-smote-t2-spring-",var_pred,"-",Sys.time(),".RData",sep=""))
